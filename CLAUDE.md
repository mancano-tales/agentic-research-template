...
- **Planos ativos**: consulte o índice de status em `0-governance/plan/README.md`.
...
| `TODO.md` | Both | Append-only task log — 3 seções (Pendente/Prospectivo/Concluído), item novo no topo, cada item com data+hora e agente/humano de criação e conclusão, e link para o plano em `0-governance/plan/` quando complexo | Toda sessão que cria, promove ou conclui uma tarefa |
| `README.md` | Humans | What the project is, repo map, how to build | Structural repo change |
| `GUIDANCE.md` / `0-governance/GUIDANCE_MAP.md` | Both | Master sitemap of directory structures and guidance documents | Repository layout or guideline change |
| `NEWS.md` (root) | Both | Intellectual changelog — history, never rewritten | Every session with a relevant decision |
| `0-governance/plan/README.md` | Both | **Status index of all plans** (ATIVO/CONCLUÍDO/…) | Any plan created or changing status |
| `0-governance/plan/*.md` | Both | Dated plans; the ATIVO ones guide current work | New plan per work round |
| `0-governance/llm-reviews/README.md` | Both | Convention for archived LLM conversations/reviews | Convention change |
...
- **Synchronized Commit Policy (Co-committing)**: Cada commit contendo mudanças de funcionalidade ou documentação deve obrigatoriamente incluir a atualização do [NEWS.md](NEWS.md) (e o status do plano em `0-governance/plan/README.md` se aplicável) na *mesma transação de commit*. Todo log de agente no `NEWS.md` deve terminar com o bloco **Metadados de Execução**:
  - **Timestamp rigor**: a data isolada não é suficiente. Todo timestamp nos artefatos de governança deste repositório — o cabeçalho de entrada no `NEWS.md` (`## YYYY-MM-DD HH:MM — Título`), o campo `**Data/Hora**` no bloco de Metadados de Execução abaixo, e os campos YAML `criado`/`concluido` em `0-governance/plan/*.md` — **devem incluir hora e minuto**, no formato `YYYY-MM-DD HH:MM`, [seu fuso horário local]. Se a hora exata não puder ser recuperada confiavelmente do log da sessão, deixe apenas a data e explique o motivo em um comentário — nunca invente um horário.
  ```markdown
...
- **Auditoria de Conversas**: Ao final de cada sessão, o agente deve exportar o histórico de conversa rodando o script:
  ...
  E registrar a nova entrada na tabela de inventário em `0-governance/llm-reviews/README.md`.
