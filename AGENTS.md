...
|---|---|---|
| `diretorio_governanca` | `close-task`, `export-conversation`, `git-cleanup`, `request-audit`, `tools/*.R` | `9-vers/` (o slot 9 da taxonomia numerada de repositórios de pesquisa). Consumidores que usam outro nome — `0-meta/` na raiz `MancanoSync` e no repo `skills` — declaram o seu aqui. Nas skills (que são texto e não executam detecção) o valor é lido **desta tabela**, pela convenção `{gov}`. Nos scripts R a resolução é automática, nesta ordem: **(1)** env var `GOV_DIR`, se setada; **(2)** detecção em disco (`0-meta` → `9-vers`); **(3)** fallback. A detecção existe para que um repo `0-meta/` tenha o default correto sem depender de ninguém lembrar de setar a env var |
